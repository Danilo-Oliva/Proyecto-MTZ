...
# --- PANTALLA HISTORIAL (Incrustada aquí para no crear otro archivo) ---
...
class VentanaHistorial(QWidget):
    def __init__(self, parent=None):
        ...
        self.tabla.setHorizontalHeaderLabels(["Fecha", "Hora", "Nombre", "Apellido", "Evento"])
        self.tabla.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.tabla.setStyleSheet("QTableWidget { background-color: white; color: #333; gridline-color: #ccc; border: 1px solid #ccc; } QHeaderView::section { background-color: #eee; color: #333; padding: 5px; }")
        layout.addWidget(self.tabla)
        
        btn_refresh = QPushButton("🔄 Actualizar Lista")
        btn_refresh.setCursor(Qt.CursorShape.PointingHandCursor)
        btn_refresh.setStyleSheet("background-color: #3498db; color: white; padding: 10px; border-radius: 5px; font-weight: bold;")
    ...
    def cargar_historial(self):
        ...
        if conn:
            ...
            for row_idx, fila in enumerate(datos):
                ...
                if "Rechazado" in fila[3] or "Vencido" in fila[3]: item_evento.setForeground(Qt.GlobalColor.red)
                elif "Ingreso" in fila[3]: item_evento.setForeground(Qt.GlobalColor.darkGreen)
...
class TarjetaDato(QFrame):
    def __init__(self, titulo, valor, color_fondo, parent=None):
        super().__init__(parent)
        self.setStyleSheet(f"QFrame {{ background-color: {color_fondo}; border-radius: 10px; }} QLabel {{ background-color: transparent; color: white; border: none; }}")
...
class DashboardWidget(QWidget):
    def __init__(self, parent=None):
        ...
        lbl_bienvenida.setStyleSheet("font-size: 28px; color: #333; font-weight: bold; margin-bottom: 20px;")
    ...
    def actualizar_metricas(self):
        ...
        if conn:
            ...
            cursor.execute("SELECT SUM(p.precio) FROM miembros m JOIN planes p ON m.plan_id = p.id WHERE m.activo = 1")
            res = cursor.fetchone()[0]
            caja = res if res else 0
            conn.close()
            self.grid.addWidget(TarjetaDato("Socios Activos", activos, "#2980b9"), 0, 0)
            self.grid.addWidget(TarjetaDato("Cuotas Vencidas", vencidos, "#c0392b"), 0, 1)
...
class PanelAdmin(QMainWindow):
    def __init__(self):
        ...
        lista_botones = [btn_inicio, btn_nuevo, btn_gestion, btn_reportes, btn_historial, btn_herramientas]
    ...
    def cambiar_pagina(self, indice):
        ...
        if indice == 0: self.pagina_inicio.actualizar_metricas()
        elif indice == 2: self.pagina_gestion.cargar_socios()
        elif indice == 3: self.pagina_historial.cargar_historial()
